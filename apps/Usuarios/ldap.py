...
class Ldap:
  ...
  def __init__(self, server=LDAP_SETTINGS['server'], 
    ...
    user_format = USER_FORMAT.format(user.lower())
  ...
  def __serialize_user(self, data):
    ...
    dep_match = re.search("OU=\w+", data["distinguishedName"])
    # si hay match remueve el texto "OU=" y retorna la dependencia, si no hay match retorna None
    dependencia = dep_match.group()[3::] if dep_match is not None else None
